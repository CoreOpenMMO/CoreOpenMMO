...
namespace COMMO.Server
{
    ...
    public class Game
    {
        ...
        public Game()
        {
            ...
            walkLock = new object();
            notifQueueLock = new object();
            combatQueueLock = new object();

            Scheduler = new Scheduler(DateTime.Now); // TODO: chose another date here?
        ...
        public WorldState Status
        {
            ...
            {
                status = value;
                Console.WriteLine($"Game world is now {status}.");
            }
        }
        ...
        public void Begin(CancellationToken masterCancellationToken)
        {
            ...
            CancelToken = masterCancellationToken;
            ...
            Scheduler.OnEventFired += ProcessFiredEvent;
        ...
        public bool ScheduleEvent(IEvent newEvent, TimeSpan delay = default(TimeSpan))
        {
            ...
            if (newEvent.EvaluateAt == EvaluationTime.OnExecute || newEvent.CanBeExecuted)
            {
                ...
                if (noDelay)
                {
                    Scheduler.ImmediateEvent(newEvent);
                    return true;
                }

                Scheduler.ScheduleEvent(newEvent, DateTime.Now + delay);
                return true;
            }
        ...
        {
            lock (combatQueueLock)
            {
                CombatQueue.Enqueue(newOp);

                Monitor.Pulse(combatQueueLock);
            }
        }
        ...
        public void NotifySinglePlayer(IPlayer player, Func<Connection, Notification> notificationFunc)
        {
            ...
            try
            {
                var conn = Connections[player.CreatureId];
        ...
        public void NotifySpectatingPlayers(Func<Connection, Notification> notificationFunc, params Location[] locations)
        {
            ...
            foreach (var spectator in allSpectating)
            {
                try
                {
                    var conn = Connections[spectator.CreatureId];
                ...
                try
                {
                    ...
                    foreach (var creature in Creatures.Values.Where(c => c.WalkingQueue.Count > 0).ToList())
                    {
                        ...
                        if (cooldownTime <= TimeSpan.Zero && creature.WalkingQueue.TryPeek(out nextTuple) && creature.NextStepId == nextTuple.Item1)
                        {
                            ...
                            if (!RequestCreatureWalkToDirection(creature, nextTuple.Item2) && creature is IPlayer)
                            {
                                ...
                                var connection = Connections[player.CreatureId];
                    ...
                    lock (walkLock)
                    {
                        if (minCoolDown != TimeSpan.MaxValue)
                        {
                            ...
                            Monitor.Wait(walkLock, actualCooldown); // there was work, but it's not time yet.
                        }
                        else
                        {
                            Monitor.Wait(walkLock); // there was no work, sleep until woken up.
                        }
                    }
                }
                catch (Exception ex)
                ...
                try
                {
                    ...
                    foreach (var creature in Creatures.Values.Where(c => c.AutoAttackTargetId > 0).ToList())
                    {
                        ...
                        if (cooldownTime <= TimeSpan.Zero)
                        {
                            ...
                            var target = GetCreatureWithId(creature.AutoAttackTargetId) as Creature;
                    ...
                    lock (attackLock)
                    {
                        if (minCoolDown != TimeSpan.MaxValue)
                        {
                            ...
                            Monitor.Wait(attackLock, actualCooldown); // there was work, but it's not time yet.
                        }
                        else
                        {
                            Monitor.Wait(attackLock); // there was no work, sleep until woken up.
                        }
                    }
                }
                catch (Exception ex)
            ...
            Parallel.ForEach(spawnsList, spawn =>
            {
                Parallel.For(0, spawn.Count, i =>
                {
                    ...
                    do
                    {
                        ...
                        Pathfind(spawn.Location, randomTile.Location, out foundLocation, (i + 1) * 100);
        ...
        private void ConnectionSweeper()
        {
            while (!CancelToken.IsCancellationRequested)
            {
                ...
                foreach (var pcKvp in Connections.Where(playerConn => !playerConn.Value.Socket.Connected).ToList())
                {
                    var player = GetCreatureWithId(pcKvp.Key) as Player;
        ...
        private void NotificationsProcessor()
        {
            while (!CancelToken.IsCancellationRequested)
            {
                ...
                while (NotificationQueue.Count == 0 || !NotificationQueue.TryDequeue(out notification))
                {
                    lock (notifQueueLock)
                    {
                        Monitor.Wait(notifQueueLock);
                    }
                }
        ...
        {
            if (sender != Scheduler || eventArgs?.Event == null)
            {
                return;
            }
        ...
        private void CombatProcessor()
        {
            while (!CancelToken.IsCancellationRequested)
            {
                ...
                lock (combatQueueLock)
                {
                    while (CombatQueue.Count == 0 || !CombatQueue.TryDequeue(out combatOp))
                    {
                        Monitor.Wait(combatQueueLock);
                    }
                }
        ...
        {
            return Map.GetCreatureIdsAt(location);
        }
        ...
        {
            return Connections.Keys.Select(creatureId => Creatures[creatureId]).Where(c => c.CanSee(location)).Cast<IPlayer>().ToList();
        }
        ...
        internal Player Login(PlayerModel playerRecord, Connection connection)
        {
            ...
            Map[Server.Map.Map.VeteranStart].AddThing(ref playerThing);
        ...
        internal bool AttemptLogout(IPlayer player)
        {
            ...
            Creature creature;
            Creatures.TryRemove(player.CreatureId, out creature);

            Connection connection;
            return Connections.TryRemove(player.CreatureId, out connection);
        }

        internal byte[] GetMapDescriptionAt(IPlayer forPlayer, Location location)
        {
            return Map.GetDescription(forPlayer, (ushort)(location.X - 8), (ushort)(location.Y - 6), location.Z, location.IsUnderground).ToArray(); // TODO: handle near top left of map edge case.
        }
        ...
        {
            return Map.GetDescription(player, fromX, fromY, currentZ, isUnderground, windowSizeX, windowSizeY).ToArray();
        }
        ...
        internal byte[] GetMapFloorsDescription(IPlayer forPlayer, ushort fromX, ushort fromY, short startZ, short endZ, byte windowSizeX, byte windowSizeY, int startingOffsetZ = 0)
        {
            ...
            {
                totalBytes.AddRange(Map.GetFloorDescription(forPlayer, fromX, fromY, (sbyte)currentZ, windowSizeX, windowSizeY, startZ - currentZ + startingOffsetZ, ref skip));
            }
        ...
        private void OnContainerContentEvent(sbyte operationType, IContainer container, byte index, IItem item)
        {
            ...
            {
                var peeker = GetCreatureWithId(peekerId) as IPlayer;
