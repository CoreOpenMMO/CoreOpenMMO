...
namespace COMMO.Security.Encryption
{
    public static class Rsa
    {
		...
		public static bool Encrypt(ref byte[] buffer, int position, bool useCipValues = true) => useCipValues ? Encrypt(CipE, CipN, ref buffer, position) : Encrypt(OtServerE, OtServerM, ref buffer, position);
        ...
        public static bool Decrypt(ref byte[] buffer, int position, int length, bool useCipValues = true)
        {
            ...
            var m1 = useCipValues ? input.ModPow(CipDp, CipP) : input.ModPow(OtServerDp, OtServerP);
            var m2 = useCipValues ? input.ModPow(CipDq, CipQ) : input.ModPow(OtServerDq, OtServerQ);
            BigInteger h;
